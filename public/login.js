    ...
    ajaxPost('/u2f_register_data.php', {"username":username}, function(resp) {
        ...
        u2f.register([resp.request], resp.signatures, function(sig) {
            ...
            ajaxPost('/complete_registration.php', {
                "username": username,
                "request_str": JSON.stringify(resp.request),
...
document.getElementById('auth_form').addEventListener("submit", function(e) {
    e.preventDefault();
    var username = document.getElementById("auth_username").value;
    ajaxPost('/u2f_auth_data.php', {"username":username}, function(resp) {
        ...
        u2f.sign(resp, function(sig) {
            ...
            ajaxPost('/complete_auth.php', {
                "username": username,
                "request_str": JSON.stringify(resp),
                "signature_str": JSON.stringify(sig)
            }, displayResponse, displayResponse);
        });
    }, displayResponse);
